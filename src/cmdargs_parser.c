...
/*   Updated: 2026/02/27 01:36:46 by dchernik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parser_loop_first_part(t_shell *msh, int opt_i, bool *f_exit);
 ...
 * If --help or --version option were
 * found we just have to exit our shell */
int	cmdargs_parser(t_shell *msh, bool *f_exit)
{
	int	opt_i;
	int	fret;
	...
	{
		fret = parser_loop_first_part(msh, opt_i, f_exit);
		if (fret == BREAK)
			break ;
